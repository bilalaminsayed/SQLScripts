...
FROM products
WHERE category_id = 2;
/* 3. For each order in the orders tables write a select statement that returns the order_id, customer_id and a calculated column named "total fees" that adds the ship_amount and tax_amount values. */
...
FROM order_items;
/* 5. Write a select statement that returns the customer_id, and one column from the customers table  named full_name that combines the last name, and the first name of each customer
...
FROM orders;
/* 11. Write a select statement that returns the customer_id, line1, line2, and phone_number columns from the addresses table for any customer located in the state of New Jersey or the city of New York. Rename the customer_id  column as "NY Area Call List".  */
...
LIMIT 0,3;
/* 14. Write a select statement that returns the product_id, the product_name, and the first 50 characters of the description from the products table. */
...
LIMIT 0, 5;
/* 17. Write a select statement that returns all columns from the addresses table for customers NOT located in Colorado, California, or Oregon. Use the IN operator to accomplish this */
SELECT *
FROM addresses
WHERE state NOT IN ('CO', 'CA', 'OR');
/* 18. List all of the orders in the orders table. Only list orders that have a card type of Visa, a tax amount of 0, and a shipping amount of 5.00 or less */
SELECT *
FROM orders
WHERE card_type = 'Visa'
AND tax_amount = 0
AND ship_amount <= 5;
/* 19. Use the REGEXP operator to return all columns and all rows from the addresses table where where the street address (line1) column ends in 'Rd.' */
SELECT *
FROM addresses
