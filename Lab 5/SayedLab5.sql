...
SELECT SUM(total)
FROM invoice
WHERE BillingCountry = 'Brazil' or 'Chile' or 'Argentina';
...
SELECT genre.name, genre.GenreId, count(*) number_of_tracks
...
SELECT AVG(track.Milliseconds), track.AlbumId, album.Title album_title, artist.artistid artist_id, artist.Name artist_name
FROM track 
...
SELECT c.supportrepid, sum(i.total)
FROM customer c
...
SELECT album.AlbumId, title, MIN(Milliseconds) shortest_track, MAX(Milliseconds) longets_track, AVG(Milliseconds) avg_track_length
FROM album 
...
SELECT e.firstname, e.lastname, count(customerid)
FROM employee e  
...
FROM artist artist 
JOIN album album ON artist.artistid = album.artistid
JOIN track track ON album.albumid = track.albumid
JOIN invoiceline invoiceline ON track.trackid = invoiceline.trackid
JOIN invoice invoice ON invoiceline.invoiceid = invoice.invoiceid
